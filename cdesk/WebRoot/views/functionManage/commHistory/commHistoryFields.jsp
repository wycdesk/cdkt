...
<body class="page" style="overflow:hidden;">
	<div class="panel-frame ps-container ps-active-y" data-ps-id="0d023947-837c-712b-ae7b-616bac5e0197">
		<div class="panel-wrap">
			<h2>
				联络历史字段<a class="js-tooltip tooltipstered"><i class="fa fa-question-circle"></i></a>
			</h2>
			<div class="kf5-section">
				<header class="kf5-list-header">
					<h3>启用的字段（${size1}）</h3>
					<a role="button" class="btn-sm green fr" href="<%=request.getContextPath()%>/commHistoryField/commHistoryFieldType"
						...
						<li>
							...
							<c:choose>
                               ...
                               <c:when test="${item.isDefault==false}">
                                   <a class="btn-sm fr" href="<%=request.getContextPath()%>/commHistoryField/fieldEdit?field=${item.key}" target="_self">编辑</a> 
	...
	<script type="text/javascript">
		...
		function model_change(type,key){
			var url = "<%=request.getContextPath()%>/commHistoryField/change/"+type+"/"+key;
		...
		setDragSort(sortCtner,sortItems,handle,function(){
		    ...
		    $.post("<%=request.getContextPath()%>/commHistoryField/ajax/",{type:"sort",ids:ids},function(data){
		    	if(data.success){
					notice.success(data.msg);
				}else{
					notice.warning(data.msg);
