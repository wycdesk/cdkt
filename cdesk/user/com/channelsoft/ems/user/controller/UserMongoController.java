...
public class UserMongoController {
	...
	public AjaxResultPo registerBase(HttpServletRequest request,DatEntUserPo po,String picCode) throws Exception{
		...
		if(picCode.equalsIgnoreCase(checkCode)){
			...
			try {
				if(userMongoService.existsEmails(po.getEntId(), po.getEmail())){
				...
				int add=userMongoService.registerBase(request,po,activeCode);
				if(add>=0){
					//用户注册成功日志
	...
	public String registerPwd(HttpServletRequest request,String code,Model model) throws ParseException{
		...
		try {
			...
			DatEntUserPo po=userMongoService.getEntUserPo(entId,code,null);
			if(po.getLoginPwd()!=null){
				if(SsoSessionUtils.isHasLogin(request)&&SsoSessionUtils.getUserInfo(request).getLoginName().equals(po.getLoginName())){
    				...
    				DatEntUserPo userPo = userMongoService.queryUserById(entId, user.getUserId());
			...
			if(time>1800){
				userMongoService.deleteUser(entId, new String[]{po.getUserId()});
				email=po.getEmail();
	...
	public AjaxResultPo setPwd(HttpServletRequest request,String userName,String nickName,String password,String code){
		...
		try {
			...
			if(StringUtils.isBlank(password)||StringUtils.isBlank(code)){
				return AjaxResultPo.failed(new Exception("密码不能为空"));
			}
			int suc=userMongoService.registerPwd(userName,nickName,password,entId,code);
			po=userMongoService.getEntUserPo(entId, code, null);
			if(po==null)
	...
	public AjaxResultPo forgetPwd(HttpServletRequest request,String email,DatEntUserPo po,String picCode) throws Exception{
		...
		if(picCode.equalsIgnoreCase(checkCode)){
			String entId=DomainUtils.getEntId(request);
			//判断邮箱是否已经注册
			if(userMongoService.existsEmails(entId, email)){
				try {
					String activeCode=MD5Util.MD5(email+new Date().getTime());
					DatEntUserPo userPo=userMongoService.getEntUserPo(entId,null,email);
					if(userPo==null){
						throw new Exception("邮箱不存在");
					}
					userPo.setActiveCode(activeCode);
					int reset=userMongoService.resetUser(userPo);
					if(reset<0){
						return AjaxResultPo.failed(new Exception("更新激活码失败"));
					}
					if(userPo.getLoginPwd()==null){
						userService.sendMailMongo(request,userPo,activeCode,false);
						...
						userService.sendMailMongo(request,userPo,activeCode,true);
						SystemLogUtils.Success("UserController", "忘记密码", email);
				...
				SystemLogUtils.Fail("UserController", "忘记密码", email, new BaseException("邮箱不存在"));
				return AjaxResultPo.failed(new Exception("邮箱不存在"));
			}
		}else{
			return AjaxResultPo.failed(new Exception("验证码错误 "));
		}
	...
	public String resetPwd(HttpServletRequest request,String code,Model model) throws ParseException{
		...
		try {
			...
			po=userMongoService.getEntUserPo(entId,code,null);
			String createStr=po.getUpdateTime();
			Date create=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(createStr);
			Date end=new Date();
			long time=(end.getTime()-create.getTime())/1000;
			if(time>3600){
	...
	public AjaxResultPo emailResend(HttpServletRequest request,String code,String email){
		...
		po.setEntId(entId);
		try {
			SendMainResponsePo resPo=userService.sendMailMongo(request,po,code,false);
	...
	public AjaxResultPo queryUserById(HttpServletRequest request,String userId,String loginType){
		try {
			String entId=DomainUtils.getEntId(request);
			DatEntUserPo po=userMongoService.queryUserById(entId, userId);
