		...
		<div class="panel-wrap">
			<form id="yw0" target="_self">
				...
				<ul class="breadcrumbs">
					<li><a href="<%=request.getContextPath()%>/commHistoryField/commHistoryfields" target="_self">联络历史字段</a></li>
					<li>编辑 联络历史字段 : ${field.componentTypeStr}</li>
				</ul>
...
<script type="text/javascript">
	...
	function formSubmit(){
		...
		$.post("<%=request.getContextPath()%>/commHistoryField/goEdit/${field.key}",param,changeCallBack,'json');
	}
	function changeCallBack(data){
		...
		location.href="<%=request.getContextPath()%>/commHistoryField/commHistoryfields";
	}
	function delField(){
		if (!confirm("确定要删除该字段吗？删除后不可恢复！")){
			return;
		}
		var url = "<%=request.getContextPath()%>/commHistoryField/change/delete/${field.key}";
