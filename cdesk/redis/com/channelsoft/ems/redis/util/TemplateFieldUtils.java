...
public class TemplateFieldUtils extends RedisBaseUtil{
	 ...
	 * @author 魏铭
	 */
	private static void setList(String entId, CacheGroup cacheGroup,List<BaseFieldPo> list) {
	...
	private static void setMap(String entId, CacheGroup cacheGroup, Map<String,List<BaseFieldPo>> map) {
		if (CacheConstants.CACHE_DATA_STATUS())
		{
			dataLock.writeLock().lock();
			try {
	...
	private static void setGroupMap(Map<String, List<BaseFieldPo>> groupMap,CacheGroup cacheGroup) {
		if (CacheConstants.CACHE_DATA_STATUS())
		{
			dataLock.writeLock().lock();
			try {
	...
	public static List<BaseFieldPo> getList(String entId,CacheGroup cacheGroup) {
		if (CacheConstants.CACHE_DATA_STATUS())
		{
			dataLock.readLock().lock();  
			try {
				List<BaseFieldPo> obj = getJedisInstance().getList(entId + "_" + LIST_KEY, cacheGroup.name,BaseFieldPo.class);
				return obj;
			} catch (Exception e) {
				e.printStackTrace();
			} finally {  
	...
	@SuppressWarnings("unchecked")
	public static Map<String,List<BaseFieldPo>>  getMap(String entId,CacheGroup cacheGroup) {
		if (CacheConstants.CACHE_DATA_STATUS())
		{
			dataLock.readLock().lock();  
			try {
				Map<String,List<BaseFieldPo>> obj = getJedisInstance().getListMap(entId + "_" + MAP_KEY, BaseFieldPo.class, cacheGroup.name);
				return obj;
			} catch (Exception e) {
				e.printStackTrace();
			} finally {  
	...
	@SuppressWarnings("unchecked")
	public static Map<String, List<BaseFieldPo>> getGroupMap(CacheGroup cacheGroup) {
		if (CacheConstants.CACHE_DATA_STATUS())
		{
			dataLock.readLock().lock();  
			try {
				Map<String,List<BaseFieldPo>> obj = getJedisInstance().getListMap(GROUP_MAP_KEY, BaseFieldPo.class,cacheGroup.name);
				return obj;
			} catch (Exception e) {
				e.printStackTrace();
			} finally {  
	 ...
	 * @param flags
	 */
	public static void getCacheFromDB(CacheGroup cacheGroup,Map<String, List<BaseFieldPo>> list, Map<String, Map<String, List<BaseFieldPo>>> map, Map<String, String> flags) {
	 ...
	 * @param flag
	 */
	public static void getCacheFromDB(CacheGroup cacheGroup,String entId, List<BaseFieldPo> list,Map<String,List<BaseFieldPo>> map, String flag) {
		if (list!=null && list.size()>0 ) {
