...
<body class="page" style="overflow:hidden;">
	<div class="panel-frame ps-container ps-active-y" data-ps-id="0d023947-837c-712b-ae7b-616bac5e0197">
           ...
           <ul class="breadcrumbs">
                <li>
                    <a target="_self" href="<%=request.getContextPath()%>/commHistoryTemplate/commHistoryTemplates">联络历史自定义分类</a>
                </li>
			...
			<div class="kf5-section">
				<header class="kf5-list-header">
					<h3>启用的字段（${size1}）</h3>
					<a role="button" class="btn-sm green fr" href="<%=request.getContextPath()%>/commHistoryTemplate/fieldType?tempId=${tempId}"
	...
	<script type="text/javascript">
		...
		function model_change(type,key){
			...
			if (type != "delete" || confirm("确定要删除该联络历史字段吗？删除后不可恢复！")){
		...
		function formSubmit(){					
			...
			$.post("<%=request.getContextPath()%>/commHistoryTemplate/goEdit", param, changeTempCallBack,'json');
		}
		/* 修改模板信息回调函数 */
		function changeTempCallBack(data){
			...
			location.href="<%=request.getContextPath()%>/commHistoryTemplate/commHistoryTemplates"; 
		}
